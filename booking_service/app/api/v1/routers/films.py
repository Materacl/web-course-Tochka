...
def create_new_film(film: FilmCreate,
...
def upload_film_image(film_id: int, file: UploadFile = File(...),
                      db: Session = Depends(get_db),
                      current_admin: User = Depends(get_current_active_admin)):
...
def delete_film_from_db(film_id: int,
                        db: Session = Depends(get_db),
                        current_admin: User = Depends(get_current_active_admin)):
...
def set_film_status(film_id: int,
...
def read_films(skip: Optional[int] = None, limit: Optional[int] = None,
